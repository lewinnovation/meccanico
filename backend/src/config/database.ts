...
export const dataSourceOptions: DataSourceOptions = {
  ...
  url:
    process.env.DB_URL ||
    `postgres://${process.env.DB_USER || "meccanico"}:${
      process.env.DB_PASSWORD || "meccanico_dev_password"
    }@${process.env.DB_HOST || "localhost"}:${process.env.DB_PORT || "5432"}/${
      process.env.DB_NAME || "meccanico"
    }`,
...
export const initializeDatabase = async (): Promise<DataSource> => {
  try {
    ...
    try {
      const { migrateCommunicationTemplates } = await import(
        "../utils/migrateCommunicationTemplates"
      );
    ...
    if (isProduction) {
      try {
        ...
        `);
        const tableCount = Number.parseInt(result[0]?.count || "0", 10);

        if (tableCount === 0) {
          console.log(
            "⚠️  No tables found in production. Running synchronize to create schema..."
          );
        ...
        console.error(
          "⚠️  Failed to check/sync database schema:",
          syncError.message
        );
